 ...
 * Este arquivo funciona como o "cérebro" de informações fixas do site.
 * Aqui centralizamos os nomes das ligas, os IDs dos times e a lógica das fotos.
 */

const GD_DATA = {
    // LEAGUES: Converte os códigos brutos que vêm da API em nomes amigáveis para o usuário ler.
    LEAGUES: {
    ...
    /**
     * POPULAR_TEAMS: Lista de times que aparecem em destaque na página inicial (Home).
     * Cada time tem seu ID oficial mapeado para facilitar a busca de estatísticas.
     */
    POPULAR_TEAMS: [
    ...
    /**
     * TEAM_LOGOS_MAP: O "Mapa da Mina". 
     * Como a nossa API não manda escudos, este dicionário liga a sigla do time (ex: RMA)
     * ao ID correto do Fotmob para a gente puxar a imagem do servidor deles.
     */
    TEAM_LOGOS_MAP: {
        ...
        // COMPETIÇÕES EUROPEIAS (OUTROS TIMES)
    ...
    // UI_CONFIG: Configurações visuais de reserva para quando algo der errado.
    UI_CONFIG: {
        FALLBACK_LOGO: 'Images/favi.svg', // Imagem padrão se o logo falhar
        AVATAR_BASE: 'https://ui-avatars.com/api/?background=3b2e7d&color=fff&bold=true' // Gerador de avatar por texto
    }
};

/**
 * getTeamLogo: A função mágica que resolve o problema dos escudos.
 * Ela tenta achar o escudo oficial; se não conseguir por ID, gera um avatar roxo com a sigla.
 */
function getTeamLogo(shortName, fullName = "") {
    ...
    // 1. Lógica de Desempates: Resolve casos onde a sigla é igual para dois times (ex: BRA de Brasil e Braga).
    const desempates = {
    ...
    // 2. Procura no mapeamento principal que criamos lá em cima.
    ...
    // 3. Fallback: Se o time não estiver no mapa, cria um avatar roxo estilizado com as letras do time.
    return `${GD_DATA.UI_CONFIG.AVATAR_BASE}&name=${cleanShort}`;
}

// Exposições Globais: Deixa as funções disponíveis para outros arquivos JS usarem.
window.GD_DATA = GD_DATA;
window.getTeamLogo = getTeamLogo;

/**
 * DEBUG SCANNER: 
 * Isso aqui é um "segurança" do código. Ele avisa no Console do navegador
 * toda vez que um time aparece no site mas não tem um logo mapeado no TEAM_LOGOS_MAP.
 */
...
window.getTeamLogo = function(shortName, fullName = "") {
    ...
    // Se o resultado for um avatar (texto), ele avisa o desenvolvedor para cadastrar o ID.
